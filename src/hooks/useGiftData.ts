...
// Updated gift data with categories
  ...
  useEffect(() => {
    const loadGifts = () => {
      try {
        ...
        if (storedGifts) {
          giftsData = JSON.parse(storedGifts);
          
          // Check if stored gifts have category field
          const hasCategory = giftsData.every(gift => 'category' in gift);
          
          if (!hasCategory) {
            // If old data without categories, use the updated initial data
            giftsData = initialGifts;
        ...
        // Extract unique categories from initial gifts
        const uniqueCategories = Array.from(new Set(initialGifts.map(gift => gift.category)));
        setCategories(uniqueCategories);
      } finally {
        setLoading(false);
      }
